...
class Application:
    def __init__(self):
        ...
        self.root.geometry("1600x900")  # Increased window width to accommodate larger reference image
        ...
        # Reference image
        try:
            ref_img_path = "D:/python_project/Sign-Language-To-Text-and-Speech-Conversion-master/Sign-Language-To-Text-and-Speech-Conversion-master/sign-language.png"
            self.ref_image = Image.open(ref_img_path)
            self.ref_image = self.ref_image.resize((400, 400), Image.Resampling.LANCZOS)  # Match hand skeleton panel size
            self.ref_photo = ImageTk.PhotoImage(self.ref_image)
            
            self.ref_label = tk.Label(self.root, text="Sign Language Reference:", font=("Courier", 15))
            self.ref_label.place(x=1120, y=115)  # Aligned with top of hand skeleton panel
            
            self.ref_panel = tk.Label(self.root, image=self.ref_photo)
            self.ref_panel.place(x=1120, y=145)  # Below label, aligned with hand skeleton
        except Exception as e:
            print(f"Could not load reference image: {e}")
            self.ref_label = tk.Label(self.root, text="Reference image not found", font=("Courier", 15))
            self.ref_label.place(x=1120, y=115)
    ...
    def video_loop(self):
        try:
            ...
            hands = hd.findHands(cv2image_copy, draw=False, flipType=True)
